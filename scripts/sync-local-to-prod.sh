...
echo "This will overwrite production transactional data. Continue? (y/N)"
read -r CONFIRM1
if [ "$CONFIRM1" != "y" ] && [ "$CONFIRM1" != "Y" ]; then echo "Aborted."; exit 0; fi

echo "WARNING: This pushes LOCAL data to PRODUCTION. Are you absolutely sure? Type YES to confirm:"
read -r CONFIRM2
...
pg_restore -d "$PROD_DB" --data-only --disable-triggers "$DUMP_FILE" || true
