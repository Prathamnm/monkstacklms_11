...
// HR Calendar is identical to Manager Calendar (Create Holiday button + modal)
// Reuses same structure — Create Holiday goes to same /api/hr/holidays endpoint
...
export default function HRCalendarPage() {
  ...
  const approvedLeaveDates = leaves.filter(l => l.status === 'APPROVED').flatMap(l => {
    const dates: string[] = []
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }} className="p-6 lg:p-8 space-y-6">
      <div className="flex items-start justify-between">
        <PageHeader title="My Calendar" description="Manage holidays and view team schedule." />
        <button onClick={() => setShowHolidayModal(true)} className="rounded-xl bg-purple-600 text-white px-4 py-2.5 text-sm font-semibold hover:bg-purple-700 transition-colors flex-shrink-0">
          + Create Holiday
        </button>
      </div>
      <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-slate-900">{format(new Date(viewYear, viewMonth, 1), 'MMMM yyyy')}</h2>
          ...
          <div className="space-y-2">
            {thisMonthHolidays.map(h => (
              <div key={h.id} className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2"><span>{h.type === 'PUBLIC' ? '🔴' : '🟡'}</span><span className="font-medium text-slate-800">{h.name}</span></div>
