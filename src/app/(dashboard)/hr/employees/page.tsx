  ...
  return (
    <div className="p-6 md:p-8 bg-[var(--color-page-bg)] min-h-screen space-y-6">
      <PageHeader
        ...
        description="View and manage employee records synced from Azure Entra ID."
        badge={filteredEmployees.length}
        actions={
          <button 
            onClick={() => setIsExportModalOpen(true)} 
            className="flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-[11px] font-bold uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm active:scale-95"
          >
      ...
      {isLoading ? (
        ...
        <EmptyState icon="👤" title="No employees found" description="Sync with Azure Entra ID to populate your workforce." />
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">ID: {emp.id.slice(0, 8).toUpperCase()}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
