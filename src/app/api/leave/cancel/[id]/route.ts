  ...
  try {
    ...
    if (leave.employeeId !== token.userId) {
      return NextResponse.json(
        { error: 'Only the leave owner can cancel this request', code: 'FORBIDDEN' },
        { status: 403 }
      )
    }
    ...
    if (leave.status === 'APPROVED' && !isBefore(new Date(), leave.startDate)) {
      return NextResponse.json({ error: 'Cannot cancel approved leave that has already started. Contact HR.', code: 'LEAVE_STARTED' }, { status: 422 })
    }
    ...
    // Post-cancel: balance reversal, audit, emails — failures must NOT return 500
    // because the cancellation IS already applied.
    try {
    ...
    for (const hr of hrEmployees) {
      ...
      console.error('[/api/leave/cancel] Post-cancel side-effect error (leave WAS cancelled):', postErr)
    }
