...
const TOAST_ID = {
  forceLogout: 'auth-callback-force-logout',
  notAuthorized: 'auth-callback-not-authorized',
  dbUnavailable: 'auth-callback-db-unavailable',
  syncFailed: 'auth-callback-sync-failed',
  generic: 'auth-callback-generic',
} as const

/** Prevents parallel runs when React Strict Mode mounts the effect twice in quick succession. */
let authCallbackPipelineBusy = false
...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (authCallbackPipelineBusy) return
    authCallbackPipelineBusy = true
    ...
    void handleCallback().finally(() => {
      authCallbackPipelineBusy = false
    })
