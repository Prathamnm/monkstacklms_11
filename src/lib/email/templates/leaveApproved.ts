interface LeaveApprovedEmailData {
  employeeName: string
  approverName: string
  startDate: string
  endDate: string
...
export function leaveApprovedTemplate(data: LeaveApprovedEmailData): string {
  return `
<!DOCTYPE html>
<html>
<body style="margin:0;padding:0;background-color:#F8FAFC;font-family:Inter,system-ui,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    <div style="background:#FFFFFF;padding:40px;border-left:1px solid #E2E8F0;border-right:1px solid #E2E8F0;">
      <div style="display:inline-block;background:#DCFCE7;border:1px solid #86EFAC;border-radius:6px;padding:6px 14px;margin-bottom:24px;">
        <span style="color:#16A34A;font-size:13px;font-weight:600;">✅ Leave Approved</span>
      </div>
      <h2 style="color:#0F172A;font-size:20px;font-weight:700;margin:0 0 16px;">
        Your leave request has been approved!
      </h2>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">
        Great news, ${data.employeeName}! Your leave request has been approved by ${data.approverName}.
      </p>
      <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:24px;margin-bottom:24px;">
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            ...
            <td style="color:#0F172A;font-size:14px;font-weight:600;padding:8px 0;">${data.startDate} – ${data.endDate}</td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="color:#64748B;font-size:13px;font-weight:500;padding:8px 0;text-transform:uppercase;letter-spacing:0.05em;">Approved By</td>
            <td style="color:#0F172A;font-size:14px;padding:8px 0;">${data.approverName}</td>
          </tr>
        </table>
      </div>
      <div style="text-align:center;">
        <a href="${data.appUrl}/employee/my-leaves" 
