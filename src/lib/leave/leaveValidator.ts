...
export function validateSandwichRule(
  startDate: Date,
  endDate: Date,
  ...
  function isNonWorkingDay(d: Date): boolean {
    return isWeekend(d) || holidaySet.has(format(d, 'yyyy-MM-dd'))
  }

  const dayBefore = subDays(startDate, 1)
  const dayAfter = addDays(endDate, 1)

  if (isNonWorkingDay(dayBefore) && isNonWorkingDay(dayAfter)) {
    let blockStart = dayBefore
    while (isNonWorkingDay(subDays(blockStart, 1))) {
      blockStart = subDays(blockStart, 1)
    }
    let blockEnd = dayAfter
    while (isNonWorkingDay(addDays(blockEnd, 1))) {
      blockEnd = addDays(blockEnd, 1)
    }
    return {
      valid: false,
      error: `Sandwich leave is not allowed. You cannot take leave on both sides of a holiday/weekend block (${format(blockStart, 'dd MMM')} – ${format(blockEnd, 'dd MMM')}). Please apply leave either before ${format(blockStart, 'dd MMM')} or after ${format(blockEnd, 'dd MMM')}.`,
    }
  }
